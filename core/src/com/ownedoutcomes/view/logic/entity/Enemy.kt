...
open class Enemy(world: World, x: Float, y: Float, val gameManager: GameManager, val spriteName: String = "goblin${MathUtils.random(enemiesAmount - 1)}", bodySize: Float = 6f, initialHealth: Int = 3) : AbstractEntity (
        world.body {
            position.set(x, y)
            linearDamping = 10f
            fixedRotation = true
...
class Boss(world: World, x: Float, y: Float, gameManager: GameManager, spriteName: String = "goblin${viableBosses.random()}") : Enemy(world, x, y, gameManager, spriteName, bodySize = 9f, initialHealth = 6) {
